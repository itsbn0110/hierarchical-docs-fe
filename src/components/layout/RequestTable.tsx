...
const RequestsTable: React.FC<RequestsTableProps> = ({ requests, loading, onApprove, onDeny, isHistory = false }) => {
    const baseColumns = [
        {
            title: "Tài nguyên",
            dataIndex: "node",
        ...
        },
        {
            ...
            dataIndex: "requester",
            key: "requester",
            render: (requester: PendingRequest["requester"]) => requester?.username || <Text type="secondary">[Ẩn danh]</Text>,
        },
        {
            title: "Quyền yêu cầu",
    ...
    const historyColumns = [
        ...baseColumns,
        {
            ...
            render: (status: string) => (
                status === 'APPROVED'
                    ? <Tag icon={<CheckCircleOutlined />} color="success">Đã chấp thuận</Tag>
                    : <Tag icon={<CloseCircleOutlined />} color="error">Đã từ chối</Tag>
            ),
        },
        {
            title: "Người xử lý",
            dataIndex: "reviewer",
            key: "reviewer",
            render: (reviewer: ProcessedRequest["reviewer"]) => reviewer?.username || <Text type="secondary">-</Text>,
        },
        {
            title: "Ngày xử lý",
            ...
            render: (_: unknown, record: PendingRequest) => (
                <Space>
                    <Tooltip title="Chấp thuận">
                        <Button type="primary" shape="circle" icon={<CheckCircleOutlined />} onClick={() => onApprove && onApprove(record._id)} />
                    </Tooltip>
                    <Tooltip title="Từ chối">
                        <Button danger shape="circle" icon={<CloseCircleOutlined />} onClick={() => onDeny && onDeny(record._id)} />
                    </Tooltip>
                </Space>
            ),
        }
    ...
    return (
        <Table
            ...
            style={{ marginTop: 24 }}
            locale={{
                emptyText: <Empty description={isHistory ? "Không có lịch sử yêu cầu nào." : "Không có yêu cầu nào đang chờ xử lý."} />,
            }}
        />
    );
};
